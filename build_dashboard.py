#!/usr/bin/env python3
"""Build dashboard HTML from data.json - no string escaping issues."""
import json
import os
import shutil

DATA_PATH = '/sessions/confident-amazing-keller/mnt/outputs/data.json'
OUTPUT_PATH = '/sessions/confident-amazing-keller/mnt/outputs/dashboard_clean.html'
WORKSPACE_PATH = '/sessions/confident-amazing-keller/mnt/对标账号月度更新/对标账号看板.html'

with open(DATA_PATH, 'r', encoding='utf-8') as f:
    data = json.load(f)

# Use a template approach: write HTML with placeholders, then fill in data
...
function renderSheetTabs() {
  ...
  DATA.accounts.forEach(function(a) {
    if (!trackSet[a.赛道]) { trackSet[a.赛道] = true; tracks.push(a.赛道); }
  });
  tracks.sort();
  var ag = DATA.stats.account_grades;
  var html = '<div class="sheet-tab" data-filter="track" data-value="" data-sheet="全部">全部<span class="count">(' + ag.total + ')</span></div>';
  tracks.forEach(function(t) {
    var count = DATA.accounts.filter(function(a) { return a.赛道 === t; }).length;
...
function applyFilters() {
  ...
  if (activeTrack) filteredAccounts = filteredAccounts.filter(function(a) { return a.赛道 === activeTrack; });
...
try:
    ...
    print(f'JSON VALID: {len(parsed["accounts"])} accounts')
